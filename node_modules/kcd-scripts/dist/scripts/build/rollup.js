...
function go() {
  ...
  if (hasTypescript && !args.includes('--no-ts-defs')) {
    ...
    for (const format of formats) {
      ...
      const outputs = isCodesplitting ? glob.sync(fromRoot(path.join(dirpath, format, '*.js'))) : [fromRoot(path.join(dirpath, filename))];
      ...
      for (const output of outputs) {
        ...
        const relativePath = path.join(path.relative(dir, process.cwd()), typeDef).replace(/\.d\.ts$/, ''); // make a .d.ts file for every generated file that re-exports index.d.ts
    ...
    const ignoredFiles = [...glob.sync(fromRoot('dist', '**/__tests__/**')), ...glob.sync(fromRoot('dist', '**/__mocks__/**'))];
...
function getPReactCommands() {
  return { ...prefixKeys('react.', getCommands()),
